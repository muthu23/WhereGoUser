    ...
    public void findViewByIdandInitialization() {
        ...
        if (provider != null) {
            ...
            if (provider.getImg().equalsIgnoreCase("http"))
                Picasso.get().load(provider.getImg()).memoryPolicy(MemoryPolicy.NO_CACHE).placeholder(R.drawable.user).error(R.drawable.user).into(profile_Image);
            else
                Picasso.get().load(URLHelper.base +"storage/app/public/"+provider.getImg()).memoryPolicy(MemoryPolicy.NO_CACHE).placeholder(R.drawable.user).error(R.drawable.user).into(profile_Image);
        }
    }
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, URLHelper.USER_PROFILE_API+"?user_id="+strUserId,
            ...
            public void onResponse(JSONObject jsonProviderObj) {
                try {
                    ...
                    Picasso.get().load(URLHelper.base + jsonProviderObj.optString("picture")).memoryPolicy(MemoryPolicy.NO_CACHE).placeholder(R.drawable.user).error(R.drawable.user).into(profile_Image);
