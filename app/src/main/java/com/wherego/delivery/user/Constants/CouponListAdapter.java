package com.wherego.delivery.user.Constants;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        try {
            discount.setText(SharedHelper.getKey(context, "currency")
                    ...
                    list.get(position).optJSONObject("promocode").optString("discount")
                    + "" + "%" );
            promo_code.setText(context.getString(R.string.the_applied_coupon)
                    + " " +
                    list.get(position).optJSONObject("promocode").optString("promo_code")
                    + ".");
            String date = list.get(position).optJSONObject("promocode").optString("expiration");
            expires.setText(context.getString(R.string.valid_until)
                    + " " +
                    getDate(date)
                    + " " +
                    getMonth(date)
